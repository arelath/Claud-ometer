...
describe('app settings helpers', () => {
  ...
  const previousSettingsDir = process.env.CLAUD_OMETER_SETTINGS_DIR;
  const previousResumeTransport = process.env.CLAUD_OMETER_RESUME_TRANSPORT;
  ...
  beforeEach(() => {
    ...
    process.env.CLAUD_OMETER_SETTINGS_DIR = settingsDir;
    delete process.env.CLAUD_OMETER_RESUME_TRANSPORT;
    vi.resetModules();
  });
  ...
  afterEach(() => {
    platformSpy?.mockRestore();
    fs.rmSync(settingsDir, { recursive: true, force: true });
    if (previousSettingsDir == null) delete process.env.CLAUD_OMETER_SETTINGS_DIR;
    else process.env.CLAUD_OMETER_SETTINGS_DIR = previousSettingsDir;
    if (previousResumeTransport == null) delete process.env.CLAUD_OMETER_RESUME_TRANSPORT;
    else process.env.CLAUD_OMETER_RESUME_TRANSPORT = previousResumeTransport;
  });
  ...
  it('lets an environment value override stored settings', async () => {
    ...
    settings.setAppSettings({ resumeTransport: 'pty' });
    process.env.CLAUD_OMETER_RESUME_TRANSPORT = 'msys2-launch';
  ...
  it('maps legacy MSYS2 tmux setting values to MSYS2 launch', async () => {
    ...
    process.env.CLAUD_OMETER_RESUME_TRANSPORT = 'msys2-tmux';
